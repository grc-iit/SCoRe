...
do
  ...
  "${EXEC_PATH}"/build/publish_test 1 "${num_messages}" "${default_message_size}" >> ./results/publish_thread_test_"${thread}" &
  ...
  #"${EXEC_PATH}"/build/subscribe_test "${url}" 40 "${num_messages}"  >> ./results/subscribe_thread_test
...
do
  ...
  "${EXEC_PATH}"/build/publish_test 1 "${num_messages}" "${size_message}" >> ./results/publish_thread_test_"${thread}" &
