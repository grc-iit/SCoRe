...
class Graph(ABC):
    ...
    # ares-comp-02 1
    # to
    # { {"ares-comp-01": 0}, {"ares-comp-02": 1}}
    def _convert_pairs_toMap(self, filename):
        a_file = open(filename, "r")
        list_of_lists = {}
        for line in a_file:
            stripped_line = line.strip()
            line_list = stripped_line.split(sep=" ")
            list_of_lists.setdefault(line_list[0], []).append(line_list[1])
        a_file.close()

        return list_of_lists

    def _make_list_unique(self, list):
        unique_list = []
        for x in list:
